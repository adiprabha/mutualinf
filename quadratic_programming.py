...
def easy_matrix_solns(r,s,t):
  ...
  # Solve sfor the equations and map to the specific ordering
  for i in range(1, 4):
      a = solns(r, s, t, x, i)
      if a is not None:
          if i == 1:
...
def transition_matrix(matrix):
  ...
  bound1 = [0, 1+10**(-6)]
  Q = 2 * np.matmul(np.transpose(coef_matrix), coef_matrix)
  c = -2 * np.matmul(np.transpose(coef_matrix), beq)
  #res = linprog(c,A_ub=aub,b_ub=bub, A_eq=coef_matrix, b_eq=beq, bounds=bound1)

  lb = np.zeros(Q.shape[1])
  ub = np.ones(Q.shape[1])
  new_coef_matrix = np.zeros((45,120),dtype=float) 
  neq_beq = np.zeros((45,),dtype=float)
  x = solve_qp(Q, c, aub, bub, new_coef_matrix, neq_beq, lb, ub, solver="osqp")
  x = np.transpose(x)
  n = np.dot(np.matmul(coef_matrix, x) - beq, np.matmul(coef_matrix, x) - beq)
  return(float(n)) 

  #print(np.matmul(coef_matrix, x) - beq)
...
matrix_4 = np.array([
[0.18308369, 0.00307044, 0.9108424],
 [0.0993292, 0.96255384, 0.34562283],
 [0.00863637, 0.58599457, 0.50132218]
])
b = transition_matrix(matrix_3)
print(b)
x = np.zeros(50)
for i in range(50):
  a = np.random.rand(3, 3)
  x[i] = transition_matrix(a)

print(x)

#intervals = [(0, 0.05), (0.05, 0.1), (0.1, 0.15), (0.15, 0.2), (0.2, 0.25), 
 #            (0.25, 0.3), (0.3, 0.35), (0.35, 0.4), (0.4, 0.45), (0.45, 0.5), (0.5, 0.55), (0.55,0.6), (0.6,0.65), (0.65,0.7), (0.7,0.75), (0.75,0.8), (0.8,0.85), (0.85,0.9),(0.9,0.95),(0.95,1), (1,1.05), (1.05, 1.1), (1.1, 1.15), (1.15,1.2)]

#counts = [sum(1 for n in x if low <= n < high) for low, high in intervals]
#print(counts)
